...
public static class GamePad {
 public enum GGAKHLLMPMM { Any=0,One=1,Two=2 } public enum LCNPGEANNDP { LeftStick,RightStick,Dpad } public enum PFENLAPGKFM { A,B,Y,X,RightShoulder } public enum HKKPDLMCPIF { LeftTrigger,RightTrigger }
 public static readonly Dictionary<(GGAKHLLMPMM,LCNPGEANNDP),Vector2> Axes=new(); public static readonly HashSet<(GGAKHLLMPMM,int)> Buttons=new();
 public static Vector2 CNNMBBLLGNE(LCNPGEANNDP s,GGAKHLLMPMM p,bool raw=false)=>Axes.TryGetValue((p,s),out var v)?v:Vector2.zero;
}
namespace Eclipse.Input {
 public static class FightControllerBindings { public static GamePad.LCNPGEANNDP MovementStick=>GamePad.LCNPGEANNDP.LeftStick; public static int Get(int a)=>a; public static bool IsPressed(int a,GamePad.GGAKHLLMPMM p)=>GamePad.Buttons.Contains((p,a)); }
...
class Program {
 ...
 static void Main(){
  var p1=new FightGamepadInput(c=>enabled,(t,c)=>Emit(A,t,c),GamePad.GGAKHLLMPMM.One); var p2=new FightGamepadInput(c=>enabled,(t,c)=>Emit(B,t,c),GamePad.GGAKHLLMPMM.One);
  Clear(); Input.Keys.Add(KeyCode.W); Input.Keys.Add(KeyCode.O); GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.One,1)); p1.Poll(true,true,false); p2.Poll(false,false,true);
  ...
  Clear(); var one=new FightGamepadInput(c=>true,(t,c)=>Emit(A,t,c),GamePad.GGAKHLLMPMM.One); var two=new FightGamepadInput(c=>true,(t,c)=>Emit(B,t,c),GamePad.GGAKHLLMPMM.Two); GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.One,0)); GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.Two,1)); one.Poll(false,false,true);two.Poll(false,false,true);Check(A[0]=="0:Punch"&&B[0]=="0:Kick","two pads not distinct");
  ...
  Clear(); var disc=new FightGamepadInput(c=>true,(t,c)=>Emit(A,t,c));GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.One,0));disc.Poll(false,false,true);Input.Devices=Array.Empty<string>();disc.Poll(false,false,true);Check(A.Count==2&&A[1]=="1:Punch","disconnect did not neutralize");Check(!FightGamepadInput.IsConnected(GamePad.GGAKHLLMPMM.One),"disconnected slot reported connected");Input.Devices=new[]{"","pad2"};Check(!FightGamepadInput.IsConnected(GamePad.GGAKHLLMPMM.One)&&FightGamepadInput.IsConnected(GamePad.GGAKHLLMPMM.Two),"IsConnected slot mapping failed");
  Clear(); var rel=new FightGamepadInput(c=>enabled,(t,c)=>Emit(A,t,c));GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.One,0));rel.Poll(false,false,true);rel.ReleaseAll();int n=A.Count;rel.ReleaseAll();Check(A.Count==n&&A[n-1]=="1:Punch","ReleaseAll not idempotent");
  Clear(); var gate=new FightGamepadInput(c=>enabled,(t,c)=>Emit(A,t,c));GamePad.Buttons.Add((GamePad.GGAKHLLMPMM.One,0));gate.Poll(false,false,true);enabled=false;gate.Poll(false,false,true);Check(A.Count==2&&A[1]=="1:Punch","enabled gate did not release active control");
